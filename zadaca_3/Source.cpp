...
struct Enviroment
{
	...
	Enviroment(const string& filename, const float& r, const int& width, const int& height) : r(r), width(width), height(height)
	{
...
Vec3f cast_ray(const Vec3f& orig, const Vec3f& dir, const Objects& objs, const Lights& lights, const Enviroment& env, unsigned int depth = 0)
{
...
void render(const Viewport& view, const Objects& objs, const Camera& cam, const Lights& lghts, const Enviroment& env, const string& filename)
{
...
int main()
{
	...
	Sphere s2(Vec3f(-2, 5, -10), 2, green);
	Cuboid surface(Vec3f(-50, -7, -30), Vec3f(50, -7.001, -10), blue);
	Cuboid c(Vec3f(-8, -7.002, -16), Vec3f(-5, -4, -13), gray);
	
	Model prism("./prism.obj", 1, Vec3f(0, 0, 1), gray);

	Objects objs = { &s1, &s2, &surface, &c, &prism };

	Light l1 = Light(Vec3f(-20, 50, 20), 1.5);
	Light l2 = Light(Vec3f(20, 30, 20), 1.8);
	Lights lights = { l1, l2 };

	Viewport view1(1024, 768, M_PI / 2);
	Viewport view2(1024, 768, 2.14 / 2);

	Camera cam1(Vec3f(0, 0, 0), Vec3f(0, 0, -1), 0);
	Camera cam2(Vec3f(0, 1, 5), Vec3f(0, 0, -1), 0);

	Enviroment env("./env.ppm", 1500, 2880, 1800);

	render(view1, objs, cam1, lights, env, "./view1.ppm");
	render(view1, objs, cam2, lights, env, "./view2.ppm");
	render(view2, objs, cam2, lights, env, "./view3.ppm");
